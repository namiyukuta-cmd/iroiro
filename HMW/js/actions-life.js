(() => {
  ...
  function board() {
    ...
    H.addLead("scrap_money", "工業通りで廃品を探し、回収所へ持ち込む");
    H.addLead("casual_work", "駅前で今日の小仕事を確認する");
    H.addHistory("掲示板から、支援・廃品回収・書類不要の小仕事という別々の生活手段を見つけた。");
    G.refresh();
  }
  ...
  function shoppingLook() {
    ...
    H.addHistory("店と裏道を把握した。店員との関係や、裏手の清掃仕事につながる可能性がある。");
    G.refresh();
  }
  ...
  function checkCasualWork() {
    ...
    H.addHistory(H.state.daily.casualOffer
      ? "搬入口に今日だけの荷下ろし手伝いが一件出ている。"
      : "今日は書類なしの小仕事は出ていない。廃品、店、支援など別の手段は残っている。");
    G.refresh();
  }
  ...
  function beg() {
    ...
    advanceTime(1, `${D.locations[id].name}で${money}円を得た。ここで続ければ周囲の注意は強くなる。`);
  }
  ...
  function rest() {
    ...
    advanceTime(1, "その場でしばらく休んだ。時間を使った代わりに疲労が戻った。");
  }
  ...
  function wash() {
    ...
    H.addHistory("身支度をした。水を使える場所を知っていること自体が生活の手札になる。");
    G.refresh();
  }
  ...
  function foodSupport() {
    ...
    H.addHistory("支援食を受け取り少し食べ、残りを持ち物に入れた。今日の食料に余裕ができた。");
    G.refresh();
  }
  ...
  function supportStart(priority) {
    ...
    closeModal();
    advanceTime(1, "相談記録が作られた。食事だけではなく、連絡先・仕事・寝場所の手続きを継続して進められるようになった。");
  }

  function supportConsult() {
    const p = H.state.progression.support;
    if (!p.stage) {
      openModal("相談で優先すること", "今日すべてを解決するのではなく、何から生活基盤を作るか決める。", [
    ...
    if (H.state.day < p.nextDay) {
      info("支援相談", `次はDAY${p.nextDay}以降。今日は街の別の仕事・人・資源を使える。`);
      return;
    }
    p.stage += 1;
    p.nextDay = H.state.day + 1;
    ...
    if (p.stage === 2) {
      ...
      text = "一時宿泊相談を一度使えるようになった。今夜の選択肢が増えた。";
    }
    if (p.stage === 4) {
      p.workAccess = true;
      H.addLead("formal_work", "労働窓口で正式な日雇いを確認する");
      text = "正式な日雇い紹介を使えるようになった。収入の経路が一つ増えた。";
    }
    advanceTime(1, text);
  }

  function openShop() {
    const ids = ["bread", "rice_ball", "bottled_water", "toothbrush", "toothpaste", "wet_wipes", "soap", "bandage"];
  ...
  function clerkWork() {
    ...
    if (H.state.stats.fatigue > 88 || H.state.stats.hunger > 90) {
      info("清掃仕事", "今日は状態が悪い。店との仕事の関係は失われない。");
      return;
    }
    G.startCleanupWork();
  }
  ...
  function clerkTalk() {
    ...
    const r = rel("clerk");
    openModal("コンビニ店員", "顔を覚えられるだけで終わる日もあれば、店の事情が見えてくる日もある。", [
      {
        label: "普通に話す",
        onClick: () => {
          ...
          closeModal();
          advanceTime(1, r.familiarity >= 3 ? "店の裏清掃を頼む日があると教えられた。" : "短い会話を重ね、顔を覚えられてきた。");
        }
      },
      {
        label: "廃棄予定の食べ物を聞く",
        ...
        onClick: () => {
          ...
          closeModal();
          advanceTime(1, ok ? "今日は廃棄前の食べ物を一つ分けてもらえた。" : "今日は渡せる物はないと言われた。関係は次の日にも残る。");
        }
      },
      {
        label: "清掃の仕事を聞く",
