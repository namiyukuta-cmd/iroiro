...
function moveTo(key){
  if(!PLACES[key]||key===state.place)return;
  state.place=key;
  advance(75);
  note(PLACES[key].name+"へ移動した。");
  toast(PLACES[key].name);
  renderAll();
}
...
function sellAll(item,unit){
  ...
  state[item]=0;state.money+=n*unit;
...
function openShop(){
  ...
  document.getElementById("backPlace").onclick=function(){renderPanel()};
}
...
function renderPanel(){
  var p=document.getElementById("panelContent");
  if(activeTab==="place"){
    var html="<div class='placeHead'><b>"+PLACES[state.place].name+"</b><span>"+PLACES[state.place].kind+"</span></div><div class='actionGrid'>";
    placeActions().forEach(function(x){html+="<button type='button' data-action='"+x[1]+"'>"+x[0]+"</button>"});
    html+="</div><div class='travelTitle'>移動先</div><div class='travelGrid'>";
    Object.keys(PLACES).filter(function(k){return k!=="road"&&k!==state.place}).forEach(function(k){
      html+="<button type='button' data-travel='"+k+"'>"+PLACES[k].name+"</button>";
    });
    html+="</div>";
    p.innerHTML=html;
    p.querySelectorAll("[data-action]").forEach(function(b){b.onclick=function(){doAction(b.dataset.action)}});
    p.querySelectorAll("[data-travel]").forEach(function(b){b.onclick=function(){moveTo(b.dataset.travel)}});
    return;
  }
...
function renderSky(){
  ...
  var isDay=t>=6&&t<18;
  var local=isDay?(t-6)/12:((t<6?t+6:t-18)/12);
  celestial.className="sunMoon "+(isDay?"sun":"moon");
  celestial.style.left=(96-local*92)+"%";
}
...
function renderScene(){
  ...
  document.getElementById("placeKind").textContent=place.kind;
}
